...
Infers active workflow (GitHub Flow / Trunk-Based / GitFlow / Release Flow / Stacked Diffs) from branch graph, protection rules, config files (`.gitflow-config`, `.graphite_config`, `.sl/`, `.git/branchless/`), and release cadence in `git tag`. Handles multi-workflow monorepos per-subtree via CODEOWNERS or `.sylph/workflow-map.yaml`.
...
Standalone: `/plugin install branch-workflow@sylph`.
...
- **Consumes** `sylph.task.boundary.detected` to drive branch creation.
- **Publishes** `sylph.workflow.detected { subtree, label, confidence }`.
