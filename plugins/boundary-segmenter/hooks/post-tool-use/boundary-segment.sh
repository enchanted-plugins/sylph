...
if command -v jq >/dev/null 2>&1 && [[ -n "$verdict" ]]; then
    ...
    printf '{"ts":"%s","event":"post_tool_use","boundary":%s,"distance":%s,"uncertain":%s,"seg_exit":%d}\n' \
        "$ts" "$boundary" "$distance" "$uncertain" "$seg_exit" \
        >> "$METRICS"
    ...
    if [[ "$boundary" == "true" ]]; then
        ...
        printf '{"ts":"%s","event":"weaver.task.boundary.detected","closed_cluster":%s,"active_cluster":%s,"distance":%s,"uncertain":%s}\n' \
            "$ts" "$closed" "$active" "$distance" "$uncertain" \
            >> "$EVENTS"
    fi
fi
