...
Weighted moving averages over preference signals, persisted via Fae-A4 atomic serialization (tempfile + rename). Tracks:
...
After 6+ weeks, W1 and W3 adapt to the developer's style. Learnings export to `shared/learnings.json` — the Gauss Accumulation network that joins Wixie F6, Raven V6, and the wider ecosystem.
...
| Script | atomic_json.py | Fae-A4 tempfile-rename pattern |
