...
Sylph's mapping:
  ...
  - get_pr / update_pr / merge_pr / close_pr → raise. Reviewers live in
    mailing-list archives; Sylph can't track those without scraping.
...
class SourceHutAdapter(HostAdapter):
    ...
    def __init__(
        ...
        self._sender = sender or self._git_config("user.email") or "sylph@localhost"
    ...
    def _resolve_list(self, repo: str) -> str | None:
        ...
        cfg = self._git_config("sylph.srht-list")
        if cfg:
            return cfg
        env = os.environ.get("SYLPH_SRHT_LIST")
        if env:
            return env
        return None
    ...
    def open_pr(self, repo, base, head, title, body, draft=True, reviewers=None):
        ...
        if not list_addr:
            raise NotImplementedHostOp(
                ...
                "constructor list_address=, `git config sylph.srht-list`, "
                "or $SYLPH_SRHT_LIST.",
            )

        # Generate patches in a tmpdir.
        with tempfile.TemporaryDirectory(prefix="sylph-srht-") as td:
            tdp = Path(td)
            try:
                subprocess.run(
                    [
    ...
    def merge_pr(self, repo, number, strategy="merge-commit"):
        raise NotImplementedHostOp(
            ...
            "the maintainer. Sylph cannot automate this.",
        )
    ...
    def _build_cover_letter(
        ...
        msg["To"] = list_addr
        msg["X-Mailer"] = "sylph/0.1.0"
        msg.set_content(
            f"{body}\n\n"
            f"---\n"
            f"This patch series was generated by Sylph "
            f"(https://github.com/enchanted-plugins/sylph).\n"
            f"Repository: {repo}\n"
        )
        return msg
