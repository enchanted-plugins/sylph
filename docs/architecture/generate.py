#!/usr/bin/env python
"""
Sylph Architecture Generator — reads the codebase and produces:
...
Adapted from wixie/docs/architecture/generate.py.
"""
...
def gen_dataflow_mermaid(plugins):
    ...
    events_map = {
        "commit-intelligence": ["sylph.commit.drafted", "sylph.commit.committed"],
        "boundary-segmenter": ["sylph.task.boundary.detected"],
        "branch-workflow": ["sylph.workflow.detected", "sylph.branch.created"],
        "pr-lifecycle": ["sylph.pr.drafted", "sylph.pr.ready", "sylph.pr.merged"],
        "sylph-gate": ["sylph.destructive.detected", "sylph.destructive.confirmed"],
        "capability-memory": ["capability_loaded", "gitlab_version_probed"],
        "ci-reader": ["sylph.ci.status.observed"],
        "sylph-learning": ["priors_loaded", "preferences_checkpointed"],
    }
    ...
    lines.append('    bus["🌐 mcp-event-bus<br/>Cross-plugin coordination"]')
    for p in plugins:
        pid = p["short"].replace("-", "_")
        lines.append(f'    {pid}_metrics --> bus')
...
<head>
<meta charset="utf-8">
<title>Sylph — Architecture Explorer</title>
...
<div class="container">
  <h1>Sylph <span>Architecture Explorer</span></h1>
