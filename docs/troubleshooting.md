...
- The hook timed out. Hooks have per-event budgets; see [shared/conduct/hooks.md](../shared/conduct/hooks.md) § Performance budget.
...
**Symptom:** `/plugin install full@weaver` succeeds but `/plugin list` doesn't show the sub-plugins.
...
**Cause:** Someone's error payload is unhelpful — see [shared/conduct/tool-use.md](../shared/conduct/tool-use.md) § Error payloads are contracts.
...
- **Bug:** reproducible, unexpected behavior. File in [Issues](https://github.com/enchanted-plugins/weaver/issues) with the bug template.
- **Question or "is this expected?":** open a thread in [Discussions](https://github.com/enchanted-plugins/weaver/discussions).
...
Then: [Discussions Q&A](https://github.com/enchanted-plugins/weaver/discussions/categories/q-a).
