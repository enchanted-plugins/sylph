...
# Crow V4 session-continuity, and falls back gracefully when missing.
set -euo pipefail
...
assert_contains "$out" "Crow V4 session-continuity data unavailable" "V4 fallback note present"
...
assert_not_contains "$out" "Crow V4 session-continuity data unavailable" "no fallback note when V4 present"
